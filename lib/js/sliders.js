 ...
 * @class TopSlider
 * @version 1.0.2
 * @author https://github.com/tophief
...
class TopSlider {
   ...
   * @param {String} prefix
   */
  constructor(el, config = {}, prefix = 'top-slider-') {
    this.#state = {
   ...
   * @param {String} prefix
   */
  static getOrCreateInstance(target, config = {}, prefix = 'top-slider-') {
  ...
  static createInstances() {
    document.querySelectorAll('[data-slider="top-slider"]').forEach((el) => {
      const { dataset } = el;
      const params = {};
  ...
  #onClick(e) {
    ...
    if (!(e.target.closest('.top-slider-btn') || e.target.closest('.top-slider-indicators'))) {
      return;
    }
...
//    * @param {String} prefix
//    */
//   constructor(el, config = {}, prefix = 'guest-slider-') {
//     this.#state = {
//       prefix, // префикс для классов
//       el, // элемент который нужно активировать как GuestSlide
//       elWrapper: el.querySelector(`.${prefix}${this.constructor.#EL_WRAPPER}`), // элемент с #CLASS_WRAPPER
//       elItems: el.querySelector(`.${prefix}${this.constructor.#EL_ITEMS}`), // элемент, в котором находятся слайды
//       elListItem: el.querySelectorAll(`.${prefix}${this.constructor.#EL_ITEM}`), // список элементов, являющиеся слайдами
//       btnPrev: el.querySelector(`.${prefix}${this.constructor.#BTN_PREV}`), // кнопка, для перехода к предыдущему слайду
...
//   /**
//    * Статический метод, который возвращает экземпляр GuestSlide, связанный с DOM-элементом
//    * @param {HTMLElement} elSlide
//    * @returns {?GuestSlide}
//    */
//   static getInstance(elSlide) {
//     const found = this.#instances.find((el) => el.target === elSlide);
//     if (found) {
//       return found.instance;
//     }
//     return null;
...
//    * @param {String} prefix
//    */
//   static getOrCreateInstance(target, config = {}, prefix = 'guest-slider-') {
//     const elSlide = typeof target === 'string' ? document.querySelector(target) : target;
//     const result = this.getInstance(elSlide);
//     if (result) {
//       return result;
//     }
//     const slide = new this(elSlide, config, prefix);
//     this.#instances.push({ target: elSlide, instance: slide });
//     return slide;
//   }

//   // статический метод для активирования элементов как GuestSlide на основе data-атрибутов
//   static createInstances() {
//     document.querySelectorAll('[data-slider="guest-slider"]').forEach((el) => {
//       const { dataset } = el;
//       const params = {};
//       Object.keys(dataset).forEach((key) => {
//         if (key === 'slider') {
...
//       e.preventDefault();
//     }
//     if (!(e.target.closest('.guest-slider-btn') || e.target.closest('.guest-slider-indicators'))) {
//       return;
//     }
//     const classBtnPrev = this.#state.prefix + this.constructor.#BTN_PREV;
//     const classBtnNext = this.#state.prefix + this.constructor.#BTN_NEXT;
